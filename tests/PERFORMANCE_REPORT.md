...
- **Quality Pipeline**：Redis SADD 去重 → Watermark 中位数纠偏 → HMAC-SHA256 签名验证 → GeoIP 补全
- **Lambda Router**：click/view/purchase → 热路（Redis Streams），其余 → 冷路（ClickHouse 异步批量写入）
- **Circuit Breaker**：Redis 连接失败时自动熔断降级到冷路
